...
// .wrangler/tmp/bundle-zWzzjI/checked-fetch.js
...
// .wrangler/tmp/bundle-zWzzjI/strip-cf-connecting-ip-header.js
...
// src/servers/ninstream.ts
function resolveNinStream(url, origin) {
...
__name(resolveNinStream, "resolveNinStream");

// src/servers/streamnin.ts
function resolveStreamnin(url, origin) {
  const base64Url = btoa(url).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
  const proxyUrl = `${origin}/api/proxy?url=${base64Url}`;
  return {
    url,
...
__name(resolveStreamnin, "resolveStreamnin");

// src/servers/filemoon.ts
var be = 512;
var lt = be - 1;
var dr = 2;
var lr = 2654435761;
var hr = 2246822519;
var re = /* @__PURE__ */ __name((t, e) => (t << e | t >>> 32 - e) >>> 0, "re");
var ht = /* @__PURE__ */ __name((t, e) => Math.imul(t, e) >>> 0, "ht");
function ye(t) {
  t[0] = t[0] + t[1] >>> 0;
  t[3] = re(t[3] ^ t[0], 16);
  t[2] = t[2] + t[3] >>> 0;
...
__name(yr, "yr");
function solvePoW(nonce, difficulty, timeoutMs = 2e4) {
  if (difficulty <= 0)
    return "0";
  const prefix = nonce + ":";
...
function selectKeyParts(playback) {
  ...
  if (t.length === 0)
    return r;
  const n = t.map((i) => Number(i)).filter((i) => Number.isInteger(i) && i >= 1 && i <= r.length).map((i) => r[i - 1]).filter((i) => typeof i === "string" && i.length > 0);
  return n.length > 0 ? n : r;
}
__name(selectKeyParts, "selectKeyParts");
function mergeKeyParts(parts) {
...
__name(mergeKeyParts, "mergeKeyParts");
async function decryptPlaybackPayload(playback) {
...
__name(decryptPlaybackPayload, "decryptPlaybackPayload");
async function resolveFilemoon(filemoonEmbedUrl) {
...
__name(resolveFilemoon, "resolveFilemoon");

// src/scraper.ts
var BASE_URL = "https://senshi.live";
...
async function getTrending() {
  return fetchJSON(`${BASE_URL}/sliders`);
}
__name(getTrending, "getTrending");
async function getSchedule() {
  return fetchJSON(`${BASE_URL}/schedule`);
}
__name(getSchedule, "getSchedule");
async function getAnimeDetails(slugOrId) {
  return fetchJSON(`${BASE_URL}/anime/${slugOrId}`);
}
__name(getAnimeDetails, "getAnimeDetails");
async function getEpisodes(animeId) {
  return fetchJSON(`${BASE_URL}/episodes/${animeId}`);
}
__name(getEpisodes, "getEpisodes");
async function getEpisodeStreams(animeId, episodeNumber, origin) {
  const embeds = await fetchJSON(`${BASE_URL}/episode-embeds/${animeId}/${episodeNumber}`);
  const results = [];
...
__name(getEpisodeStreams, "getEpisodeStreams");
...
__name(decodeBase64Url2, "decodeBase64Url");
function encodeBase64Url(str) {
  return btoa(str).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}
__name(encodeBase64Url, "encodeBase64Url");
function rewriteM3U8(content, playlistUrl, origin) {
...
app.get("/api/proxy", async (c) => {
  ...
  let targetUrl;
  try {
    targetUrl = decodeBase64Url2(targetEncoded);
  } catch {
...
// .wrangler/tmp/bundle-zWzzjI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-zWzzjI/middleware-loader.entry.ts
